...
class ImportArmorTypeFromCSV extends CSVImportHelper
{
    const FILE_ACCESS_PATH = '/Depository/Starfinder/CommonPieces/ArmorType.csv';
    protected static $defaultName = 'app:import:add:sf:common:armor-type';
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($this->specColl as $row) {
            ...
            $typeFinder = $this->documentManager->getRepository(ArmorType::class)->findOneBy(['name' => $type->getName()]);
            if ($typeFinder) {
                continue;
            }
            $this->documentManager->persist($type);
            $this->importsLogger->info('[' . $count . ']Persisted armor type: ' . $type->getName());
        }
    ...
    private function composeFromRow(array $row)
    {
        ...
        // $type->???($row[0]); is for entity like on items or on vehicules... depends on the future.
