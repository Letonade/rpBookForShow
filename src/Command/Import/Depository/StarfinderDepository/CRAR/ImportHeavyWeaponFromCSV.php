...
class ImportHeavyWeaponFromCSV extends CSVImportHelper
{
    const FILE_ACCESS_PATH = '/Depository/Starfinder/CRAR/HeavyWeapon.csv';
    protected static $defaultName = 'app:import:add:sf:crar:hw';
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($this->saColl as $row) {
            $count++;
            $hw = $this->composeFromRow($row);
            $this->documentManager->persist($hw);
            $this->importsLogger->info('[' . $count . ']Persisted hw: ' . $hw->getName());
        }
