...
class ImportLongarmFromCSV extends CSVImportHelper
{
    const FILE_ACCESS_PATH = '/Depository/Starfinder/CRAR/Longarm.csv';
    protected static $defaultName = 'app:import:add:sf:crar:la';
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($this->saColl as $row) {
            $count++;
            $la = $this->composeFromRow($row);
            $this->documentManager->persist($la);
            $this->importsLogger->info('[' . $count . ']Persisted la: ' . $la->getName());
        }
    ...
    private function composeFromRow(array $row)
    {
        ...
        $item->setCapacityAndUsages($this->stringMapper->getCapacitiesAndUsagesFromString($row[10], $row[11],
            $this->depository));
